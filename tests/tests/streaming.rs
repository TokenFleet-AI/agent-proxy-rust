...
async fn test_should_stream_sse_frames() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_raw(sse_body.clone(), "text/event-stream"),
        )
        .mount(&upstream)
        .await;
    ...
    assert!(
        body.contains("data: "),
        "SSE response must contain data frames"
    );
    assert!(
        body.contains("message_start"),
        "must contain message_start event"
    );
    assert!(
        body.contains("content_block_start"),
        "must contain content_block_start event"
    );
    assert!(
        body.contains("content_block_delta"),
        "must contain content_block_delta events"
    );
    assert!(
...
async fn test_should_receive_sse_frames_individually() {
    ...
    for frame in &frames {
        assert!(
            ...
            "each frame must start with 'data: ', got: {frame:.50}",
        );
    }
    ...
    assert!(
        delta_count > 0,
        "must have at least one content_block_delta frame"
    );
}
...
async fn test_should_parse_usage_from_streaming_response() {
    ...
    for line in sse_body.lines() {
        if line.contains("message_delta")
            && line.contains("usage")
            && let Some(json_start) = line.find('{')
        {
