...
async fn test_should_compress_request_with_large_tools() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::anthropic_response("msg_compress", "Analysis complete", 500, 200),
        ))
        .mount(&upstream)
        .await;
...
async fn test_should_not_compress_small_requests() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::anthropic_response("msg_nocompress", "Simple response", 10, 20),
        ))
        .mount(&upstream)
        .await;
...
async fn test_should_skip_compression_when_disabled() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::anthropic_response("msg_disabled", "No compression", 200, 100),
        ))
        .mount(&upstream)
        .await;
...
async fn test_tools_increase_request_size() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::anthropic_response("msg_size", "OK", 100, 50),
        ))
        .mount(&upstream)
        .await;
