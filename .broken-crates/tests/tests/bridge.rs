...
async fn test_should_bridge_anthropic_request_to_openai_upstream() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::openai_response("chatcmpl-001", "Hello from OpenAI!", 20, 15),
        ))
...
async fn test_should_bridge_openai_request_to_anthropic_upstream() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::anthropic_response("msg_bridge_002", "Bridged response", 30, 25),
        ))
...
async fn test_should_map_models_in_bridge_conversion() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1/chat/completions"))
        .respond_with(ResponseTemplate::new(200).set_body_json(
            common::openai_response("chatcmpl-mapped", "Mapped response", 12, 8),
        ))
        .mount(&upstream)
        .await;
